package ru.practicum.stats.statsservice.repository;
...
public interface StatsRepository extends JpaRepository<Stats, Long> {
    ...
    @Query("SELECT new ru.practicum.ewmservice.stats.statsdto.OutHitsDto(s.app, s.uri, COUNT(distinct s.ip)) " +
            "FROM Stats AS s " +
    ...
    @Query("SELECT new ru.practicum.ewmservice.stats.statsdto.OutHitsDto(s.app, s.uri, COUNT(s.ip)) " +
            "FROM Stats AS s " +
            "WHERE s.timestamp BETWEEN ?1 AND ?2 " +
            "GROUP BY s.app, s.uri " +
            "ORDER BY COUNT(s.ip)")
